...
class AddBloodPressureController extends GetxController
  ...
  Rx<BloodPressureType> bloodPressureType =
      BloodPressureType.normal.obs;
  ...
  void onEdit(BloodPressureModel bloodPressureModel) {
    ...
    bloodPressureDate.value =
        DateTime.fromMillisecondsSinceEpoch(
            _bloodPressure!.dateTime!);
    _updateDateTimeString(bloodPressureDate.value);
    systolic.value = _bloodPressure?.systolic ?? 0;
    diastolic.value = _bloodPressure?.diastolic ?? 0;
    bloodPressureType.value = _bloodPressure?.bloodType ??
  ...
  void onShowBloodPressureInfo() {
    showAppDialog(
        context, TranslationConstants.bloodPressure.tr, '',
  ...
  Future onSelectBloodPressureDate() async {
    final result = await onSelectDate(
        context: context,
        initialDate: bloodPressureDate.value);
    if (result != null) {
      bloodPressureDate.value = bloodPressureDate.value
          .update(
              year: result.year,
              month: result.month,
              day: result.day);
  ...
  Future onSelectBloodPressureTime() async {
    ...
    if (result != null) {
      bloodPressureDate.value =
          bloodPressureDate.value.update(
        hour: result.hour,
  ...
  void onSelectSys(int newSys) {
    ...
    systolic.value = sys;
    if (sys < 90) {
      bloodPressureType.value =
          BloodPressureType.hypotension;
      ...
      bloodPressureType.value =
          BloodPressureType.hypertensionStage1;
    } else if ((sys >= 140 && sys <= 180)) {
      bloodPressureType.value =
          BloodPressureType.hypertensionStage2;
    } else {
      bloodPressureType.value =
          BloodPressureType.hypertensionCrisis;
    }
  }
  ...
  Future addBloodPressure() async {
    ...
    final bloodPres = BloodPressureModel(
        ...
        dateTime:
            bloodPressureDate.value.millisecondsSinceEpoch);
