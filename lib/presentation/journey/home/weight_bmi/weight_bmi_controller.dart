...
class WeightBMIController extends GetxController with DateTimeMixin, AlarmDialogMixin {
  ...
  void exportData() async {
    ...
    if(appController.isPremiumFull.value) {
      ...
      if(appController.userLocation.compareTo("Other") != 0) {
        ...
        int cntPressExport = prefs.getInt("cnt_export_weight_bmi") ?? 0;

        if (cntPressExport < 2) {
          _doExportData();
          prefs.setInt("cnt_export_weight_bmi", cntPressExport + 1);
        } else {
  ...
  _doExportData() async {
    ...
    for (final item in bmiList) {
      ...
      Map initialGender = AppConstant.listGender
          .firstWhere((element) => element['id'] == (item.gender ?? '0'), orElse: () => AppConstant.listGender.first);
  ...
  void onSetAlarm() {
    showAddAlarm(context: context, alarmType: AlarmType.weightAndBMI, onPressCancel: Get.back, onPressSave: _onSaveAlarm);
  ...
  void _generateDataChart() {
    ...
    Map mapGroupData =
        groupBy(bmiList, (p0) => DateFormat('dd/MM/yyyy').format(DateTime.fromMillisecondsSinceEpoch(p0.dateTime ?? 0)));
    DateTime? minDate;
    DateTime? maxDate;
    mapGroupData.forEach((key, value) {
      ...
      if (((value ?? []) as List).isNotEmpty) {
        ...
        listDataChart.add({'date': handleDate, 'value': bmiModel.bmi, 'timeStamp': bmiModel.dateTime});
        weightDataChart.add({'date': handleDate, 'value': bmiModel.weightKg, 'timeStamp': bmiModel.dateTime});
      }
    });
